...
# Resolve the repo root (one level up from the api/ directory)
ROOT_DIR = Path(__file__).resolve().parent.parent

# Mount all static assets from the repo root
...
async def serve_html(page: str):
    html_path = ROOT_DIR / f"{page}.html"
    if html_path.exists():
...
async def serve_knowledge_base_js():
    return FileResponse(str(ROOT_DIR / "knowledge_base.js"), media_type="application/javascript")
